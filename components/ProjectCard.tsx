...
interface ProjectCardProps {
  ...
  onLinkClick: () => void;
  onSubLinkClick?: (subName: string) => void;
}
...
export function ProjectCard({
  ...
  const hasSubLinks = (project.subLinks?.length ?? 0) > 0;
  ...
  // ── Shared card chrome (className/style/animate/transition) ────────────

  const cardClassName = `group relative block w-full py-5 px-6 rounded-2xl backdrop-blur-xl border-2 overflow-hidden transition-colors duration-300 ${
      ...
      <div className="relative z-10">
        ...
        {hasSubLinks ? (
          <a
            ...
            onClick={onLinkClick}
            className="group/title inline-flex items-center justify-center gap-2"
          >
            <span
            ...
            <svg
              className={`w-5 h-5 opacity-70 group-hover/title:translate-x-1 transition-transform ${textPrimary}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {/* Sub-links — small cards linking to individual games/tools within this project */}
        {hasSubLinks && (
          <div className="mt-4 grid grid-cols-3 gap-2">
            {project.subLinks!.map((sub) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => onSubLinkClick?.(sub.name)}
              ...
              >
                <span className={`block text-xs font-bold ${textPrimary}`}>
                  {sub.name}
                </span>
                {sub.description && (
                  <span
                    className={`block text-[10px] mt-0.5 leading-snug opacity-80 ${textSecondary}`}
                  >
                    {sub.description}
                  </span>
                )}
